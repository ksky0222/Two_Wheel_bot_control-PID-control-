...
int Ena = 4; int IN1 = 5; int IN2 = 6; 
int Enb = 7; int IN3 = 8; int IN4 = 9; 
...
int R_value,L_value;
int vel_max = 53; int vel_min = 38;
int vel_max_neg = 460; int vel_min_neg =400;
int value_max = 255; int value_min = 100;

int input_1 = 0; int input_2 = 0;
double R_current_Time, R_elapsed_Time, R_previous_Time;
double L_current_Time, L_elapsed_Time, L_previous_Time;
...
int R_u_max = 255; int R_u_min = 100;
int L_u=0; int L_u_sat=0; int L_temp_u = 0;
int L_u_max = 255; int L_u_min = 100;
double Kp = 8;
double Ki = 0.06;
double Kd = 0.2;
double Kaw = 0.2;
...
double compute_R_vel(){
  ...
  return R_temp_vel = 0.3456*1250*R_elapsed_pos_rms/(50*48); // rpm으로 변환 60*1000*edge/t*ppr*4
...
double compute_L_vel(){
  ...
  if(Serial.available()>0){ //Serial 플로터에 입력되는 값이 있으면 elapsed_pos_square_sum을 초기화 // elapsed_pos가 제곱되어 누적되기 때문에 값이 입력되어도 입력값으로 바로 가지 않는다.
   ...
   return L_temp_vel = 0.3456*1250*L_elapsed_pos_rms/(50*48); // rpm으로 변환 60*1000*edge/t*ppr*4
}
...
double compute_R_PID() {
  ...
  R_current_Time = millis();
  R_elapsed_Time = R_current_Time - R_previous_Time;
  R_previous_Time = R_current_Time;
  
  R_current_err = input_2 - R_vel;
  R_delta_err = R_current_err - R_pre_err;
  R_pre_err = R_current_err;
  R_sum_err += (R_current_err-Kaw*(R_u-R_u_sat))*R_elapsed_Time;
  R_diff_err = R_delta_err / abs(R_elapsed_Time);  
...
double compute_L_PID(){
  ...
  L_elapsed_Time = L_current_Time - L_previous_Time;
  L_previous_Time = L_current_Time;
  
  L_current_err = input_1 - L_vel;
  L_delta_err = L_current_err - L_pre_err;
  L_pre_err = L_current_err;
  L_sum_err += (L_current_err-Kaw*(L_u-L_u_sat))*L_elapsed_Time;
  L_diff_err = L_delta_err / abs(L_elapsed_Time);  
 ...
 void loop() {
  ...
  if(input_2>0){
   analogWrite(IN3,R_u_sat); analogWrite(IN4, 0);
  }
  else if(input_2 ==0){
    analogWrite(IN3,0); analogWrite(IN4,0);
 }
  else{
   analogWrite(IN3,0); analogWrite(IN4, -R_u_sat);
  }


  if(input_1>0){
   analogWrite(IN1, 0); analogWrite(IN2, L_u_sat);
  }
  else if(input_1 ==0){
   analogWrite(IN1, 0); analogWrite(IN2, 0);
  }
  else{
   analogWrite(IN1, -L_u_sat); analogWrite(IN2, 0);
  }
