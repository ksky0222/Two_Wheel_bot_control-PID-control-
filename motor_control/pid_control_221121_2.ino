...
void check_L_B(){
 ...
 void timerISR() {
  ...
  R_vel = 125*R_elapsed_pos_rms/50; // rpm으로 변환 60*1000*edge/t*ppr*4
  ...
  if(Serial.available()>0){ //Serial 플로터에 입력되는 값이 있으면 elapsed_pos_square_sum을 초기화 // elapsed_pos가 제곱되어 누적되기 때문에 값이 입력되어도 입력값으로 바로 가지 않는다.
    ...
    L_vel = 125*L_elapsed_pos_rms/50; // rpm으로 변환 60*1000*edge/t*ppr*4


  if(R_u>0) {
  if(R_u >= R_u_max) R_u_sat =R_u_max;
  else if( R_u < R_u_max && R_u_min < R_u) R_u_sat = R_u; 
  else R_u_sat=R_u_min;
  }
  else {
  if(R_u <= -R_u_max) R_u_sat = -R_u_max;
  else if( R_u > -R_u_max && -R_u_min> R_u) R_u_sat = R_u;
  ...
  if(L_u>0) {
   if(L_u >= L_u_max) L_u_sat =L_u_max;
   else if( L_u < L_u_max && L_u_min < L_u) L_u_sat = L_u; 
   else L_u_sat =L_u_min;
  }
  else {
    if(L_u <= -L_u_max) L_u_sat = -L_u_max;
    else if( L_u > -L_u_max && -L_u_min> L_u) L_u_sat = L_u;
